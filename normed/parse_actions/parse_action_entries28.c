...
{
	v->arr[1911] = (t_pae){.action = {.shift = {.type = ACT_TY_SHIFT, \
...
ACT_TY_REDUCE, .symbol = aux_sym_command_repeat2, .child_count = 2, \
	...
	v->arr[1914] = (t_pae){.action = {.shift = {.type = ACT_TY_SHIFT, \
	.state = (7619), .repetition = true}}};
	v->arr[1916] = (t_pae){.entry = {.count = 1, .reusable = false}};
	v->arr[1917] = (t_pae){.action = {.shift = {.type = ACT_TY_SHIFT, \
	.state = (559)}}};
